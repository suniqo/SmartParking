...
Adicionalmente nótese que el número de celdas dentro del conjunto $\overline{B}_n(\bar{o}) = { (x, y): dist(\bar{o}, \bar{x}) <= n}$, por lo discutido, será $4·1 + 4·2 + \dots + 4·n = 4·\displaystyle\sum_{i=1}^{n} i = 4·\frac{n(n + 1)}{2} = 2·n·(n + 1)$
