...
Sean $dist = n$, la distancia dada y &nbsp; $\bar{v_1} = ( i, j )$ &nbsp; una coordenada en la posición &nbsp; $k \in \lbrace0, \dots, \frac{4·n}{4} - 1 \rbrace$ al ordenar las $4·n$ coordenadas a distancia $n$ del origen en sentido antihorario  
...
Sin embargo, el BubbleSort tiene una complejidad de $O(i^2)$ en cada iteración $i$, luego su complejidad tras todas sus llamadas será de $O(n^2) · O(n) = O(n^3)$. Por tanto, la complejidad total del método será, en el peor caso, de $O(n^3) + O(n^2) \approx O(n^3)$. Esta complejidad se podría bajar fácilmente a $O(n^2·log(n))$ utilizando otro algoritmo de ordenación como el *QuickSort* o el *MergeSort*.
