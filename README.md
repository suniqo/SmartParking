...
Adicionalmente nótese que el número de celdas dentro del conjunto $\overline{B_n}(\bar{o}) = \lbrace \bar{x}: dist(\bar{o}, \bar{x}) \leq n\rbrace$, por lo discutido, será: $$1 + 4·1 + 4·2 + \dots + 4·n = 1 + 4·\displaystyle\sum_{i=1}^{n} i = 4·\frac{n·(n + 1)}{2} + 1 = 2·n·(n + 1) + 1$$
...
Sean $dist = n$, la distancia dada y &nbsp; $\bar{v_1} = ( i, j )$ &nbsp; una coordenada en la posición &nbsp; $k \in \lbrace0, \dots, n - 1 \rbrace$ al ordenar las $4·n$ coordenadas a distancia $n$ del origen en sentido antihorario  
...
Para concluir, calculamos la complejidad total del método ***gestionarSolicitudReserva()*** en el peor caso, sumando la complejidad de cada uno de sus métodos, ya que la complejidad del resto de operaciones es despreciable.
...
Sin embargo, el BubbleSort tiene una complejidad de $O(n^2)$ en cada iteración $i$, luego su complejidad tras todas sus llamadas será de $O(n^2) · O(n) = O(n^3)$. Por tanto, la complejidad total del método será, en el peor caso, de $O(n^3) + O(n^2) \approx O(n^3)$. Esta complejidad se podría bajar fácilmente a $O(n^2·log(n))$ utilizando otro algoritmo de ordenación como el *MergeSort*.
