...
Sin embargo, el BubbleSort tiene una complejidad de $O(i^2)$ en cada iteración $i$, luego su complejidad tras todas sus llamadas será de $O(n^3)$. Por tanto, la complejidad total del método será, en el peor caso, de $O(n^3) + O(n^2) \approx O(n^3)$. Esta complejidad se podría bajar fácilmente a $O(n^2)$ utilizando otro algoritmo de ordenación como el *QuickSort* o el *MergeSort*.
